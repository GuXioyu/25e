...
static GPTIMER_Regs* const timer_list[] = {TIMA0, TIMA1, TIMG0, TIMG6, TIMG7, TIMG8, TIMG9, TIMG12, TIMG14};

static uint16 encoder_dir[TIM_MAX] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
...
int16 encoder_get_count (timer_index_enum index)
{
    ...
    {
        count = gpio_get_level((gpio_pin_enum)encoder_dir[index]) == 1 ?  count : -count;
    }
    // else
    // {
...
{
    zf_assert((TIM_G8 == index) || (TIM_G9 == index));
...
void encoder_dir_init (timer_index_enum index, encoder_channel1_enum lsb_pin, gpio_pin_enum dir_pin)
{
    ...
    // 保存方向引脚
    encoder_dir[index] = (uint16)dir_pin;
