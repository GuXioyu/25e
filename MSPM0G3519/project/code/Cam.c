...
uint16_t cam_laser_x;			//激光 X 坐标，单位：像素
uint16_t cam_laser_y;			//激光 Y 坐标，单位：像素
uint16_t cam_prelaser_x;		//激光 X 坐标，单位：像素
uint16_t cam_prelaser_y;		//激光 Y 坐标，单位：像素
...
void Cam_ReadXY(void)    //读取图像坐标并更新全局坐标
{
    ...
    if (Tag != NULL && strcmp(Tag, "cam") == 0)    		//确认当前帧为视觉坐标帧
    {
		...
		char *Value3 = strtok(NULL, ", ");   			//解析 X 坐标字符串
        char *Value4 = strtok(NULL, ", ");    			//解析 Y 坐标字符串
		char *Value5 = strtok(NULL, ", ");   			//解析 X 坐标字符串
        char *Value6 = strtok(NULL, ", ");    			//解析 Y 坐标字符串
		//字段完整时分别更新目标坐标
        if (Value1 != NULL && Value2 != NULL)    		//确认坐标字段完整
        {
			...
			int IntValue3 = atoi(Value3);  				
            cam_laser_x = IntValue3;          			
            int IntValue4 = atoi(Value4);    			
            cam_laser_y = IntValue4;            		
			
			int IntValue5 = atoi(Value5);  				
            cam_prelaser_x = IntValue5;          			
            int IntValue6 = atoi(Value6);    			
...
uint16_t Cam_GetXY(uint8_t Axis, uint8_t Type)    //读取指定类型与轴的已保存坐标
{
	...
	else if (Type == CAM_LASER)
	{
		if (Axis == 1)return cam_laser_x;
		if (Axis == 2)return cam_laser_y;
	}
	
	else if (Type == CAM_PRELASER)
	{
		if (Axis == 1)return cam_prelaser_x;
		if (Axis == 2)return cam_prelaser_y;
	}
