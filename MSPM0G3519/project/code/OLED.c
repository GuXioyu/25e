...
#define OLED_SCL_PIN                B23             // OLED I2C SCL 引脚
#define OLED_SDA_PIN                B24 	            // OLED I2C SDA 引脚
#define OLED_SCL_IOMUX_INDEX        IOMUX_PINCM23   // B8 对应的 IOMUX 配置索引
#define OLED_SDA_IOMUX_INDEX        IOMUX_PINCM24   // B9 对应的 IOMUX 配置索引
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	gpio_set_level(OLED_SCL_PIN, BitValue);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	gpio_set_level(OLED_SDA_PIN, BitValue);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
	for (uint32_t i = 0; i < 30; i++)
	{
		__NOP();
...
void OLED_GPIO_Init(void)
{
	...
	gpio_init(OLED_SCL_PIN, GPO, GPIO_HIGH, GPO_OPEN_DTAIN);
	gpio_init(OLED_SDA_PIN, GPO, GPIO_HIGH, GPO_OPEN_DTAIN);
	IOMUX->SECCFG.PINCM[OLED_SCL_IOMUX_INDEX] |= IOMUX_PINCM_PIPU_ENABLE;
	IOMUX->SECCFG.PINCM[OLED_SDA_IOMUX_INDEX] |= IOMUX_PINCM_PIPU_ENABLE;
	/*释放SCL和SDA*/
	OLED_W_SCL(1);
	OLED_W_SDA(1);
}
