...
#define LINE_SPEED_DIFFERENCE (10) /* 偏离中心时单侧轮速相对基础速度的最大补偿量。 */
#define LINE_FORK_FINISH_ANGLE_DEG (65.0f) /* 直角岔道完成判定角度阈值，单位为度。 */
...
static void Line_CheckForkType(void) /* 判断左直角岔道并在转角完成后退出岔道状态。 */
{
    ...
    if (angle_diff <= -LINE_FORK_FINISH_ANGLE_DEG)
    {
